  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,.95)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", width: "90vw", maxWidth: 900, aspectRatio: "16/10" }}>
        <img src={photos[idx]} alt={`Foto ${idx + 1} de ${photos.length}`} style={{ width: "100%", height: "100%", objectFit: "contain", borderRadius: 8 }} />
        {photos.length > 1 && <>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
        {photos.map((p, i) => <div key={i} onClick={() => setIdx(i)} role="button" tabIndex={0} aria-label={`Ver foto ${i + 1}`} onKeyDown={(e) => e.key === 'Enter' && setIdx(i)} style={{ width: 60, height: 40, borderRadius: 6, overflow: "hidden", border: i === idx ? "2px solid #fff" : "2px solid transparent", cursor: "pointer", opacity: i === idx ? 1 : 0.5 }}><img src={p} alt={`Miniatura foto ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>)}
      </div>
    </div>
  );
};
