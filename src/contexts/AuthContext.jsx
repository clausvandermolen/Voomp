...
export function AuthProvider({ children }) {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles', filter: `id=eq.${uid}` }, (payload) => {
        ...
        setUser(prev => prev ? {
          ...
          lastNameP: data.last_name_p ?? prev.lastNameP,
          lastNameM: data.last_name_m ?? prev.lastNameM,
  ...
  const fetchProfile = async (uid, sessionUser) => {
    ...
    if (data) {
      setUser({
        ...
        lastNameP: data.last_name_p,
        lastNameM: data.last_name_m,
  ...
  const updateProfile = async (updates) => {
    ...
    const dbUpdates = {};
    const map = {
      firstName: 'first_name', lastNameP: 'last_name_p', lastNameM: 'last_name_m',
